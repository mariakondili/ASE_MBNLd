...
###~~~ Run DESeq2 for genes by HTSeq-counts, after HISAT2.Alignment ~~~~###
...
counts_dir <- paste0(data_dir,"Cutadapt/HISAT2_aligned/HTSeq_counts/")
...
####---> FIND common Genes of AAVGFP-vs-Saline & MBNLdecoy-vs-Saline

# signif_genes_dct_et_aavgfp <- with(DE_tab.ann,
#                                     subset(geneName ,
#                                     (padj_MBNLdVsCTRL < 0.05 & padj_AAVGFPvsCTRL < 0.05 )))
#
# write.table(signif_genes_dct_et_aavgfp, "Signif_Genes_Common_in_AAVGFP_&_MBNLDecoy_vs_Saline.txt",sep=",",quote=F)
#
# ###---> Filter DE_tab from these genes :
# DE_tab.ann.filtaav <- with(DE_tab.ann, subset(DE_tab.ann, (! geneName %in% signif_genes_dct_et_aavgfp ) ))
###--->  Using The filtered-AAV genes :
# D <- DE_tab.ann.filtaav
...
## FOR FILTERED-AAV GENES :
# plot_name <- paste0(plots_dir,"Expression_PHeatmap_log2fc1_padj0.05_zscoredCounts_",nrow(sigCounts.z) ,"_Filt-AAV_genes.png")
# main_title = "Signif.Diff/ly expressed Genes in MBNLd-vs-CTRL(filt.common AAV-genes)"
